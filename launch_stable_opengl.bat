@echo off
setlocal
title CinnaCupQuest Safe OpenGL Play Launcher
set "GODOT_EXE=D:\Godot\Godot_v4.3-stable_win64_console.exe"
set "PROJECT_DIR=%~dp0"
if "%PROJECT_DIR:~-1%"=="\" set "PROJECT_DIR=%PROJECT_DIR:~0,-1%"
set "LOG_DIR=%PROJECT_DIR%\.godot-user"
set "LOG_FILE=%LOG_DIR%\play_stable_opengl.log"
set "CONSOLE_LOG=%LOG_DIR%\play_stable_opengl_console.log"
set "ARG_FILE=%LOG_DIR%\play_stable_opengl_args.txt"
...
(
  ...
  echo SafeEntry=Use this launcher or the desktop CinnaCupQuest play shortcut, not the raw Godot GUI exe.
) > "%ARG_FILE%"

"%GODOT_EXE%" --disable-crash-handler --display-driver windows --rendering-driver %RENDERING_DRIVER% --rendering-method gl_compatibility --render-thread safe --single-window --resolution 1280x720 --max-fps 60 --log-file "%LOG_FILE%" --path "%PROJECT_DIR%" > "%CONSOLE_LOG%" 2>&1
>> "%ARG_FILE%" echo ExitCode=%ERRORLEVEL%
if not "%ERRORLEVEL%"=="0" (
  echo.
  echo CinnaCupQuest failed to start. See:
  echo %CONSOLE_LOG%
  echo %LOG_FILE%
  pause
)
